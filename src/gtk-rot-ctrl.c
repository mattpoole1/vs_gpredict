...
static gboolean get_state(GtkRotCtrl * ctrl)
{
    gchar          *buff, **vbuff;
    gchar           buffback[128];
    gboolean        retcode;
    ...
    if (retcode)
    {
        ...
        else
        {
            ...
            if (strncmp(vbuff[0], "C", 1) == 0)
            {
                ctrl->calibrating = TRUE;
            } else if ((strncmp(vbuff[0], "O", 1) == 0) || (strncmp(vbuff[0], "R", 1) == 0) || (strncmp(vbuff[0], "S", 1) == 0))
            {
...
/**
 * Command device to auto-calibrate
 *
 * \param ctrl Pointer to the GtkRotCtrl widget
 *
 ...
 * 
 */
static gboolean set_cal(GtkRotCtrl * ctrl)
{
    gchar          *buff;
    gchar           buffback[128];
    gboolean        retcode;
    ...
    if (retcode == TRUE)
    {
        ...
        if (retval != 0)
        {
            ...
            sat_log_log(SAT_LOG_LEVEL_ERROR,
                        _
                        ("%s:%d: rotctld returned error %d (%s)"),
...
static void track_toggle_cb(GtkToggleButton * button, gpointer data)
{
    ...
    gboolean        locked;
    sat_log_log(SAT_LOG_LEVEL_DEBUG,
                _("%s: Track Button Clicked"), __func__);
...
/**
 * Manage click signals (calibrating)
 * 
...
static void rot_locked_cb(GtkToggleButton * button, gpointer data)
{
    ...
    if (!gtk_toggle_button_get_active(button))
    {
        ...
        gtk_widget_set_sensitive(ctrl->calibrate, (!ctrl->tracking && ctrl->engaged));
